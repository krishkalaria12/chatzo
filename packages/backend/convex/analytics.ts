...
import { MODELS_SHARED } from './config/models';
...
export const getMyUsageStats = query({
  ...
  handler: async (ctx, { timeframe }) => {
    ...
    const events = await ctx.db
      .query('usageEvents')
      .withIndex('by_user_days_since_epoch', q => q.eq('userId', user.id))
      .filter(q => q.gte(q.field('daysSinceEpoch'), startDay))
      .collect();
...
export const getMyUsageChartData = query({
  ...
  > => {
    ...
    if (timeframe === '1d') {
      ...
      const events = await ctx.db
        .query('usageEvents')
        .withIndex('by_user_timestamp', q => q.eq('userId', user.id))
