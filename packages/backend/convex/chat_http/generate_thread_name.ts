...
export const generateThreadName = async (
  ...
  const modelData = await getModel(ctx, settings.titleGenerationModel, userId);
  if (modelData instanceof ChatError) return modelData;
  const { model } = modelData;
  if (model.modelType === 'image')
    return new ChatError('bad_request:api', 'Why is your title generation model an image model?');
