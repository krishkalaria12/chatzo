...
import { MODELS_SHARED, type RegistryKey, type SharedModel } from './config/models';
import { NonSensitiveUserSettings, UserSettings } from './schemas/settings';
      ...
      for (const [providerId, providerData] of Object.entries(args.generalProviders)) {
        if (providerData) {
          const existingProvider =
            settings.generalProviders?.[providerId as keyof typeof settings.generalProviders];
...
export const updateUserSettingsPartial = mutation({
  args: {
    ...
    searchProvider: v.optional(v.union(v.literal('firecrawl'), v.literal('serper'))),
