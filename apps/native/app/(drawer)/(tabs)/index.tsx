...
export default function TabOne() {
  ...
  const testConnection = async () => {
    ...
    try {
      ...
      setConnectionStatus('failed');
      Alert.alert('Connection Error', error instanceof Error ? error.message : 'Failed to connect to Convex');
    }
  };
  ...
  const testGoogleAI = async () => {
    try {
      ...
      Alert.alert('Google AI Error', error instanceof Error ? error.message : 'Failed to test Google AI');
    }
  };
  ...
  const handleSubmitMessage = async (e?: any) => {
    ...
    if (input.trim() && !isLoading) {
      try {
        ...
        Alert.alert('Submit Error', error instanceof Error ? error.message : 'Failed to send message');
      }
    }
  };
    ...
    return (
      <Container>
        <View className="flex-1 justify-center items-center p-6">
          <Text className="text-red-500 text-center text-lg font-medium mb-4">
            Chat Error
          </Text>
