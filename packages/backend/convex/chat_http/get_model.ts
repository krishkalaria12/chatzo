...
export const getModel = async (ctx: ActionCtx, modelId: string, clerkId: string) => {
  const user = await ctx.runQuery(internal.lib.identity.internalGetUserByClerkId, {
    clerkId,
  });
  if (!user) throw new ChatError('unauthorized:chat', 'User not found');
  ...
  const sortedAdapters = model.adapters.sort((a: RegistryKey, b: RegistryKey) => {
